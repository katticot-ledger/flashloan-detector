...
export async function fetchFlashLoanTransactions(
  ...
  const transactions = await Promise.all(
    events.map(async (event) => {
      ...
      const isWBTC = event.args.asset.toLowerCase() ===
...
async function displayFlashLoanTransactions(
  ...
  if (transactions.length === 0) {
    ...
    transactions.forEach((tx) => {
      const isWBTC =
        tx.asset.toLowerCase() === '0x2260fac5e5542a773aa44fbcfedf7c193bc2c599';
      const decimals = isWBTC ? 8 : 18;
      ...
      console.log(
        `Premium: ${ethers.utils.formatUnits(tx.premium)} ${tx.currency}`,
      );
    });
  }
}
